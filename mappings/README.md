## Mapping files

This directory contains information about molecular mappings for molecules in the Martini 3 metabolome.

In the [map_files](map_files) subdirectory, `.map` files used to map atomistic structures using [Fast-Forward](https://github.com/fgrunewald/fast_forward)
are available for every molecule.

In the file [mappings.pdf](mappings.pdf) file, images of each mapping have been generated using [CGSmiles](https://github.com/gruenewald-lab/CGsmiles). 
Where the CGSmiles string is available, it has been noted in database LINK DATABASE

